...
Continue with the same rules as `/lisa`: ask one focused question at a time, update the draft with `lisa_update_draft`, and finalize only when the user asks to finish. On finalization, call `lisa_finalize`, output exactly `SPEC COMPLETE`, and stop.
